...
if(lb[row]==-Infinity){finiteB.play();}
else{document.getElementById("l"+(m+1)+","+col).className="orangeval";
...
if(ub[row]==Infinity){finiteB.play();}
else{document.getElementById("u"+(m+2)+","+col).className="purpleval";
 ...
 function ck_opt()
  {
  	...
  	for (j=mmn+1; j<=n; j++) {element=document.getElementById("c"+j);
  		if (element.className=="pinkval") {
  			optflag = false;
  		}
}
for (j=1; j<=n; j++) {element=document.getElementById("l"+(m+1)+","+j);element1=document.getElementById("u"+(m+2)+","+j);
...
d_infeas_flag=true;	
  ...
  {
  	var i, j;
  	var p_infeas_flag1=false;
	var p_infeas_flag2=false;
  	...
  	for (i=1; i<=m; i++) {
	   ...
	   if ((vb[i] > 1e-10*ub[i])) {
  		...
  		for (j=mmn+1; j<=n; j++) {
  		    if (a[i][j] <  -0.00001) {
  			p_infeas_flag2 = false;
  			break;
  		    }
  		}
	   }
  		if (p_infeas_flag1 == true || p_infeas_flag2== true) break;
  	
  	}
  
  	if (p_infeas_flag1 == true || p_infeas_flag2== true) {
  		// reset();
  		meg.play();
  	} else {
  		bark.play();
